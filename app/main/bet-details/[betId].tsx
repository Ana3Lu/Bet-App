    ...
    <View style={{ flex: 1 }}>
          ...
          participants.map((p) => (
            <View key={p.id} style={styles.card}>
              ...
              {user?.role === "ADMIN" && bet.status === "ACTIVE" && (
                <TouchableOpacity
                  style={[styles.cardButton, { backgroundColor: "#4caf50", alignSelf: "center" }]}
                  onPress={() =>
                    Alert.alert("Confirm Winner", `Set ${p.player_id?.name} as winner?`, [
      ...
      {showAnimation && (
        <View style={styles.animationOverlay}>
          <LottieView
            source={require("../../../assets/animations/Marketing.json")}
            autoPlay
            loop={false}
...
const styles = StyleSheet.create({
  ...
  infoLabel: { color: "#fff", fontWeight: "bold", alignSelf: "flex-start" },
  infoValue: { color: "#adb4ffff" },
  cardButton: { paddingVertical: 8, paddingHorizontal: 20, borderRadius: 20, marginTop: 15, marginBottom: 10 },
  animationOverlay: {
    position: "absolute",
    top: 0,
    left: 0,
